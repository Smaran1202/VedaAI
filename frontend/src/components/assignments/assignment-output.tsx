...
import { ActionCard } from "@/components/assignments/action-card";
import { GenerationProgress } from "@/components/assignments/generation-progress";
import { OutputSkeleton } from "@/components/assignments/output-skeleton";
import { QuestionPaperStudio } from "@/components/assignments/question-paper-studio";
import { downloadAssignmentPdf } from "@/services/assignment.service";
import { useAssignmentStore } from "@/store/assignment-store";
import { useCurrentUser } from "@/hooks/use-current-user";
import { canRegeneratePapers } from "@/lib/roles";
...
export function AssignmentOutput({ id }: { id: string }) {
  ...
  const canRegenerate = canRegeneratePapers(role);
      ...
      {paper ? (
        <>
          ...
          <QuestionPaperStudio
            assignmentId={id}
            paper={paper}
            versions={selectedAssignment.versions}
            canEdit={canRegenerate}
            uploadedMaterialSummary={selectedAssignment.extractedContent}
            onAssignmentUpdate={setSelectedAssignment}
          />
        </>
      ) : (
        <GenerationProgress
